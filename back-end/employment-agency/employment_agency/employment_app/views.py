...
class ViecLamViewSet(viewsets.ViewSet,
    ...
    @action(methods=['get'], detail=True, url_path='by-recruiter')
    def by_recruiter(self, request, *args, **kwargs):
        parmas = kwargs
        print('parmas ', parmas)
        job = ViecLam.objects.filter(nha_tuyen_dung_id=parmas['pk'])
        serializer = ViecLamSerializer(job, many=True)
